...
class ReputePost(scrapy.Spider):
    ...
    custom_settings = {
        "LOG_LEVEL": "INFO",
        "FEEDS": {
            "s3://scrapy-buck/Repute Post Data.csv": {"format": "csv", "encoding": "utf8", "overwrite": True},
        }
    }
    ...
    def login(self):
        ...
        try:
            ...
            # logging.info(f"CSRF Name: {verification_token}")
        except AttributeError:
            logging.info("Failed to find CSRF tokens.")
            raise AttributeError
        login_response_cookies = requests.utils.dict_from_cookiejar(session.cookies)
        if login_response_cookies.get('.AspNetCore.Antiforgery.6PmDcb9J8Ys') != None:
            for cookie in self.cookies:
                if cookie['name'] == '.AspNetCore.Antiforgery.6PmDcb9J8Ys':
                    cookie['value'] = login_response_cookies['.AspNetCore.Antiforgery.6PmDcb9J8Ys']
        ...
        if login_response_cookies.get('.AspNetCore.Identity.Application') != None:
            ...
            for k, v in login_response_cookies.items():
                    if k not in ['.AspNetCore.Antiforgery.6PmDcb9J8Ys']:
                        found = False
                        for cookie in self.cookies:
                            if cookie['name'] == k:
                                cookie['value'] = v
